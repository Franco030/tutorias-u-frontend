...
export function GoogleLoginButton() {
  ...
  const isConfigured = Boolean(
    googleClientId && googleClientId !== "TU_GOOGLE_CLIENT_ID",
  );
  ...
  const loginConGoogle = useGoogleLogin({
    onSuccess: async (codeResponse) => {
      ...
      try {
        ...
        setLocalError(
          err?.message || "Error al autenticar con el servidor de TutoriasU.",
        );
      } finally {
    ...
    onError: (error) => {
      console.error(
        "[GoogleLoginButton] Falló la autenticación con Google:",
        error,
      );
    ...
    <div className="w-full flex flex-col items-center">
      {!isConfigured && (
        <div className="w-full mb-2 p-2.5 rounded-xl bg-burgundy-50 dark:bg-burgundy-950/40 border border-burgundy-200 dark:border-burgundy-800 text-burgundy-800 dark:text-burgundy-300 text-xs flex items-start gap-2">
          ...
          <span>
            <strong>Nota:</strong> Define <code>VITE_GOOGLE_CLIENT_ID</code> en
