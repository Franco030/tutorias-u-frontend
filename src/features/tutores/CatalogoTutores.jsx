  ...
  useEffect(() => {
    const fetchTutores = async () => {
      try {
        ...
        // Fallback a array por si el backend lo envuelve en algo más
    ...
    if (!rating || rating === 0) {
      return (
        <div className="flex items-center gap-1">
          <Star className="w-4 h-4 text-granite-300 dark:text-deep-space-blue-700" />
          <span className="text-xs text-granite-500 dark:text-deep-space-blue-400">Sin calificación</span>
        </div>
      );
    }
    return (
      <div className="flex items-center gap-1">
        ...
        <span className="text-sm font-medium text-granite-700 dark:text-white">
          {rating.toFixed(1)}
        </span>
      </div>
    );
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="relative max-w-xl">
          ...
          <input
            ...
            className="w-full rounded-xl border border-granite-200 bg-white pl-11 pr-4 py-3 text-sm text-granite-900 outline-none transition-colors placeholder:text-granite-400 focus:border-deep-space-blue-500 dark:border-deep-space-blue-800 dark:bg-deep-space-blue-900/40 dark:text-white dark:placeholder:text-deep-space-blue-400 dark:focus:border-emerald-500"
          />
        </div>
        ...
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center space-y-4">
             <div className="w-10 h-10 rounded-full border-4 border-deep-space-blue-200 border-t-deep-space-blue-600 dark:border-deep-space-blue-800 dark:border-t-emerald-400 animate-spin"></div>
             <p className="text-granite-500 dark:text-deep-space-blue-300">Cargando tutores...</p>
          </div>
        ) : error ? (
          <div className="rounded-xl border border-red-500/40 bg-red-500/10 p-6 text-center text-red-600 dark:text-red-400">
            {error}
          </div>
        ) : tutoresFiltrados.length === 0 ? (
          <div className="p-12 text-center bg-white dark:bg-deep-space-blue-900/20 border border-granite-200 dark:border-deep-space-blue-800 rounded-3xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {tutoresFiltrados.map((tutor) => (
              <button
                ...
                className="flex flex-col text-left bg-white dark:bg-deep-space-blue-900/20 border border-granite-200 dark:border-deep-space-blue-800 hover:border-deep-space-blue-400 dark:hover:border-emerald-500 hover:shadow-lg dark:hover:shadow-emerald-500/10 transition-all rounded-3xl p-6 overflow-hidden"
              >
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-granite-900 dark:text-white line-clamp-1">
                      {tutor.nombre}
                    </h3>
                    <div className="mt-1">{renderStars(tutor.calificacionPromedio)}</div>
                  </div>
                </div>

                <div className="mt-2 flex-1 w-full">
                  <h4 className="text-xs font-medium text-granite-500 dark:text-deep-space-blue-400 uppercase tracking-wider mb-2">
                    Especialidades
                  </h4>
                  <div className="flex flex-wrap gap-1.5">
                    {tutor.materias?.slice(0, 3).map((materia, i) => (
                      <span
                        key={i}
                        className="px-2 py-1 bg-granite-100 dark:bg-deep-space-blue-800/50 text-granite-700 dark:text-deep-space-blue-200 text-xs rounded-full border border-granite-200 dark:border-deep-space-blue-700/50"
                      >
                        {materia}
                      </span>
                    ))}
                    {tutor.materias?.length > 3 && (
                      <span className="px-2 py-1 bg-granite-50 dark:bg-deep-space-blue-900/50 text-granite-500 dark:text-deep-space-blue-400 text-xs rounded-full border border-granite-200 dark:border-deep-space-blue-800">
